...
func CacheGetRandomSatisfiedChannel(param *RetryParam) (*model.Channel, string, error) {
	...
	if param.TokenGroup == "auto" {
		...
		for i := startGroupIndex; i < len(autoGroups); i++ {
			...
			if len(exclude) > 0 {
				channel, _ = model.GetRandomSatisfiedChannelExclude(autoGroup, param.ModelName, exclude, filters)
			} else {
		...
		if len(exclude) > 0 {
			if channel == nil {
				channel, err = model.GetRandomSatisfiedChannelExclude(param.TokenGroup, param.ModelName, exclude, filters)
			}
		} else {
			if channel == nil {
