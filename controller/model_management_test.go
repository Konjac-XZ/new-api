...
func modelManagementDB(t *testing.T, kind, dsn string) *gorm.DB {
	...
	require.NoError(t, database.AutoMigrate(&model.Model{}, &model.Vendor{}, &model.Channel{}, &model.ChannelBreakerState{}, &model.ChannelTestConfig{}, &model.Ability{}, &model.Option{}, &model.User{}, &model.AuditLog{}))
			...
			for _, rule := range []int{model.NameRuleExact, model.NameRulePrefix, model.NameRuleContains, model.NameRuleSuffix} {
				t.Run(fmt.Sprintf("rule_%d_exact_names_only_atomic_and_cached", rule), func(t *testing.T) {
					...
					for i, original := range channels {
						...
						if i == 0 {
							emptyMapping := "{}"
							original.ModelMapping = &emptyMapping
						}
						assert.Equal(t, original, after, "only routing fields change")
					}
