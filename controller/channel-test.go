...
func convertRequestForAdaptor(c *gin.Context, info *relaycommon.RelayInfo, adaptor relaychannel.Adaptor, request dto.Request) (any, error) {
	...
	default:
		...
		default:
			return nil, errors.New("invalid chat request type")
		}
	}
}
...
func testChannelForHealthCheck(ctx context.Context, channel *model.Channel, testUserID int, allowDisable bool, disableThreshold int64) channelTestSummary {
	...
	if allowDisable && isChannelEnabled && shouldBanChannel && channel.GetAutoBan() && !dynamicBreakerEnabled {
	...
	if result.localErr == nil && !dynamicBreakerEnabled && !isChannelEnabled && service.ShouldEnableChannel(newAPIError, channel.Status) {
...
func ScheduledTestChannels() {
	scheduledTestChannelsOnce.Do(func() {
		...
		for {
			...
			if !operation_setting.GetMonitorSetting().ScheduledTestChannelEnabled || !common.AutomaticDisableChannelEnabled {
				continue
			}
