...
func shouldMonitorRequest(relayInfo *relaycommon.RelayInfo, relayFormat types.RelayFormat) bool {
	if relayInfo == nil {
		return false
	}
...
func Relay(c *gin.Context, relayFormat types.RelayFormat) {
	...
	defer func() {
		if newAPIError != nil {
			...
			if common.IsDownstreamContextDone(c.Request.Context()) {
				logger.LogInfo(c, fmt.Sprintf("relay aborted (downstream gone): %s", common.LocalLogPreview(newAPIError.Error())))
				return
			}
	...
	requestBodyStorage, bodyErr := common.GetBodyStorage(c)
	if bodyErr != nil {
	...
	for ; retryParam.GetRetry() <= common.RetryTimes; retryParam.IncreaseRetry() {
		...
		for attempt := 0; attempt < channelRetryAttempts; attempt++ {
			...
			if _, err := requestBodyStorage.Seek(0, io.SeekStart); err == nil {
				c.Request.Body = io.NopCloser(requestBodyStorage)
			}
			...
			processChannelError(c, *types.NewChannelError(channel.Id, channel.Type, channel.Name, channel.ChannelInfo.IsMultiKey, common.GetContextKeyString(c, constant.ContextKeyChannelKey), channel.GetAutoBan()), newAPIError, relayInfo, geminiFreeTierSuppressed)
...
func processChannelError(c *gin.Context, channelError types.ChannelError, err *types.NewAPIError, relayInfo *relaycommon.RelayInfo, suppressAutoDisable ...bool) {
