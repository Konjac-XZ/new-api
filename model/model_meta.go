...
func GetConfiguredModelChannels() (map[string][]int, error) {
	...
	if err := DB.Select("id", "models", "model_mapping").Find(&channels).Error; err != nil {
		return nil, err
	}
	configured := make(map[string][]int)
	for _, channel := range channels {
		for _, name := range normalizeLookupValues(channel.GetRoutableModels()) {
...
func DeleteModelMetadata(ids []int, removeFromChannels, removePricing bool) (ModelDeleteResult, error) {
	...
	deleteRecords := func(tx *gorm.DB) error {
		...
		if removeFromChannels {
			...
			if err := lockForUpdate(tx).Select("id", "models", "model_mapping", "status", "group", "priority", "weight", "tag").Order("id").Find(&channels).Error; err != nil {
				return err
			}
			for _, channel := range channels {
