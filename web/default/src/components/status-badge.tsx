...
function normalizeStatusVariant(
  ...
  if (variant === 'orange' || variant === 'yellow') return 'warning'
  if (variant === 'blue' || variant === 'purple') return 'info'
  return variant
}
...
export function StatusBadge({
  ...
  const badgeContent = children ?? label
  const isCopyable = copyable !== false && Boolean(copyText)
  ...
  const variant = normalizeStatusVariant(variantProp) ?? 'neutral'
  const leadingIcons: React.ReactNode[] = []
  const trailingIcons: React.ReactNode[] = []
  const labelChildren: React.ReactNode[] = []

  for (const child of React.Children.toArray(badgeContent)) {
  ...
  return useRender({
    ...
    props: mergeProps<'span'>(
      {
        className: cn(
          ...
          isCopyable && 'cursor-copy hover:brightness-95',
          className
        ),
        ...(isCopyable
          ? {
              'aria-label': ariaLabel ?? `${t('Copy')}: ${copyText}`,
          ...
          : { 'aria-label': ariaLabel, onClick, title }),
        children: (
          <>
            {leadingIcons}
    ...
    render: isCopyable ? (render ?? <button type='button' />) : render,
    state: {
      appearance,
      size,
      slot: 'status-badge',
