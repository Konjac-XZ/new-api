...
function buildWsUrl(accessToken: string | null): string {
  ...
  if (envBase) {
    try {
      ...
      const url = new URL(`${protocol}//${parsed.host}${basePath}/api/monitor/ws`)
  ...
  const url = new URL(`${protocol}//${window.location.host}/api/monitor/ws`)
  if (accessToken) url.searchParams.set('access_token', accessToken)
  return url.toString()
}
...
export function useMonitorWs(options: UseMonitorWsOptions = {}) {
  ...
  const connect = useCallback(() => {
    ...
    wsRef.current = ws
  }, [accessToken, closeWs, handleMessage])
